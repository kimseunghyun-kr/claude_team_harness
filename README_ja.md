...
> ステータス: **Preview** — `harness.toml [deliberation].enabled = true` で opt-in。orchestration モデルと persona 契約は v1.0 までに変更される可能性あり。

単一 LLM インスタンスの Plan→Work→Review ループに加えて、Harness は **複数 persona deliberation モード** を提供する。`Sketchboard.md` という共有ドキュメント上で異なる persona Agent が議論し、人間 chair が epoch 境界で ratify する。これは additive — Plans.md と既存 skill は無修正。

```bash
/harness-deliberate init "スケールだけで AGI に到達するか？"
/harness-deliberate run        # budget 尽きるか全員 abstain するまで bidding loop
/harness-deliberate review     # chair が diff + contested sections + bid distribution を見る
/harness-deliberate ratify     # ratified ref を進めて次 epoch を開く
```

仕組み:

- **Sketchboard.md は Plans.md とは別の artifact。** Plans.md はタスク管理、Sketchboard.md は deliberation HEAD。
- **通信は task assignment ではなく state-observation。** persona は毎ターン Sketchboard.md を読み、書くか abstain するかを決める。読むだけ（abstain）も valid。
- **Bidding turn model。** 各 epoch に固定の commit budget（既定 5）。各 slot で persona は relevance score を bid し、最高 bid が slot を取る。bid `0` で abstain。budget 尽きるか全員 abstain で epoch close。
- **Epoch review は code diff ではなく Sketchboard の contested section を chair に提示する。** chair は ratify / request-revision / edit-and-ratify を選ぶ。

サンプル persona は [agents/personas/](agents/personas/) に同梱（`scaling-optimist`, `architecture-skeptic`, `bias-auditor`）。自分の persona は [persona contract](agents/personas/_persona-contract.md) に従って追加可能。

v0.1 では **未対応**（後続 version へ deferral）: persona ごとの git branch、eavesdrop / 1-to-1 talk（v0.2）、CRDT および確率的 relational graph（v0.3）。v0.1 の contested-section 検出は keyword heuristic で、false positive / false negative が [tests/test-sketchboard-conflict-detection.sh](tests/test-sketchboard-conflict-detection.sh) で regression-tested 済み — v0.2 で LLM ベースの stance classifier に置換予定。

詳細: [skills/harness-deliberate/SKILL.md](skills/harness-deliberate/SKILL.md)。
